...
export class AuthService {
  ...
  let identifiant = informations.identifiant || informations.username;
  let { password } = informations;
  ...
  if (resultMatching) {
    ...
    //  Retourner UNIQUEMENT les infos nécessaires (jamais password/salt)
    return {
      access_token: token,
      id:           user.id,
